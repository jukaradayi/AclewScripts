...
def write_rttm(rttm_out, basename_per_spkr, basename_whole):
    ...
    for spkr in rttm_out:
        with open(basename_per_spkr + '_' + spkr.strip()  + '.rttm', 'w') as fout:
...
if __name__ == '__main__':
    ...
    parser.add_argument('output_folder_whole',
            help=''' Folder to put whole RTTM in, to evaluate using score.py''')
    parser.add_argument('output_folder_per_spkr',
            help=''' Folder to put RTTM per spkr in, to evaluate using score.py''')
    ...
    for fold in os.listdir(args.input_folder):
        for fin in os.listdir(os.path.join( args.input_folder, fold)):
            ...
            basename_per_spkr = os.path.join(args.output_folder_per_spkr,  '_'.join(fin.split('_')[0:3]))
            basename_whole = os.path.join(args.output_folder_whole,   '_'.join(fin.split('_')[0:3]))
